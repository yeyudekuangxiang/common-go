...
type APIEventCategory struct {
	...
	Icon            string            `json:"icon" gorm:"type:varchar(500);not null;default:'';comment:分类图标"`     //分类图标
	Sort            int               `json:"sort" gorm:"type:int4;not null;default:0;comment:排序 从大到小"`           //排序
	Link            EventCategoryLink `json:"link" gorm:"foreignKey:EventCategoryId;references:EventCategoryId"`
}

func (APIEventCategory) TableName() string {
	return "event_category"
}
