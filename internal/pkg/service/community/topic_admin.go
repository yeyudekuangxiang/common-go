...
func (srv TopicAdminService) UpdateTopic(topicId int64, title, content string, tagIds []int64, images []string) error {
	//查询记录是否存在
	topicModel, err := srv.topicModel.FindOneTopic(repository.FindTopicParams{TopicId: topicId})
	if err != nil {
...
func (srv TopicAdminService) SoftDeleteTopic(topicId int64, reason string) (*entity.Topic, error) {
	...
	if err != nil {
		if err == entity.ErrNotFount {
			return nil, errno.ErrCommon.WithMessage("帖子不存在")
		}
		return nil, err
	}
...
func (srv TopicAdminService) DownTopic(topicId int64, reason string) (*entity.Topic, error) {
	//查询数据是否存在
	topic, err := srv.topicModel.FindOneTopicAndTag(repository.FindTopicParams{
		TopicId: topicId,
	})
	if err != nil {
		if err == entity.ErrNotFount {
