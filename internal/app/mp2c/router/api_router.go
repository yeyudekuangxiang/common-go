...
func apiRouter(router *gin.Engine) {
	...
	authRouter.Use(middleware.Auth2(), middleware.Throttle())
	{
		userRouter := authRouter.Group("/user")
		{
			...
			userRouter.GET("/get-yzm-2b", apiutil.Format(api.DefaultUserController.CheckYZM2B)) //获取验证码2b
			userRouter.GET("/check-yzm-2b", apiutil.Format(api.DefaultUserController.GetYZM2B)) //校验验证码2b
