...
func adminRouter(router *gin.Engine) {
	...
	adminRouter.Use(middleware.AuthAdmin())
	{
		...
		adminRouter.POST("/coupon/send", apiutil.Format(admin.DefaultPrivateController.SendCoupon))
		...
		articleRouter := adminRouter.Group("/topic")
		{
			...
			articleRouter.POST("/delete", apiutil.Format(admin.DefaultTopicController.Down))     //文章删除 软删除
